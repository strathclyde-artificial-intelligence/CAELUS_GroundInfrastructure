...
def test_load_hospital_from_json():
    json_str = '{"id": "1", "name": "Hospital 1", "location_long": 1.0, "location_lat": 2.0, "address":"some_address", "type": 4, "maxChargingCapacity": 50, "maxStorageCapacity": 75, "isChargingStation": true}'
    json_obj = json.loads(json_str)
    hospital = Hospital.from_json(json_obj)
    assert hospital is not None

def test_load_charging_station_from_json():
    json_str = '{"id": "1", "name": "Charging Station 1", "location_long": 1.0, "location_lat": 2.0, "type": 3, "address":"some_address", "maxChargingCapacity": 50, "maxStorageCapacity": 75, "isChargingStation": true}'
    json_obj = json.loads(json_str)
